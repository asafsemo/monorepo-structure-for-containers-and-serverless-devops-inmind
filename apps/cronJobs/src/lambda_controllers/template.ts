...
import { handleError, validateMandatoryObjectFields } from '@pkg/lib-core';
...
const service = new ServiceGeneric(awsLogger);
...
export const awsLambdaHandler = async (
	...
	try {
		...
		return buildApiGatewayOkResponse({ status: 'ok' });
	} catch (error) {
